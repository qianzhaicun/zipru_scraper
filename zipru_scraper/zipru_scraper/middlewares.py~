...
class ThreatDefenceRedirectMiddleware(RedirectMiddleware):
    ...
    def _redirect(self, redirected, request, spider, reason):
        ...
        logger.debug('Zipru threat defense triggered for {request.url}')
    ...
    def wait_for_redirect(self, url = None, wait = 0.1, timeout=10):
        ...
        logger.error('Maybe {self.dryscrape_session.url()} isn\'t a redirect URL?').format(self.dryscrape_session.url())
    ...
    def solve_captcha(self, img, width=1280, height=800):
        ...
        logger.debug('Solved the Zipru captcha: "{captcha}"')
